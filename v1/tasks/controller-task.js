...
let run = function (param) {
    ...
    for(let table of param.tables){
        ...
        result.data.push({
            templatePath: path.join(__dirname, '..', 'templates', 'controller-task-template.txt'),
            savePath: path.join(process.cwd(), 'src', 'controller', `${table.tableAliasLow}-controller.js`),
            param: table
        });
    }
